...
public class NodePointMarker : MonoBehaviour
{
    public enum PointType
    {
        ...
        MustPass,    // 範囲に入ったら必ず経由したい（isMustPass=true）
    ...
    private static readonly HashSet<Vector2Int> mustPassCells = new HashSet<Vector2Int>();

    public static bool IsExcludedCell(Vector2Int cell) => excludeCells.Contains(cell);
    public static bool IsMustPassCell(Vector2Int cell) => mustPassCells.Contains(cell);

    // PlayMode設定によっては static が残ることがあるので保険でクリア（任意だがおすすめ）
    ...
    private void Awake()
    {
        ...
        else if (pointType == PointType.MustPass)
        {
            mustPassCells.Add(cell);
        }
    }

    private void OnDestroy()
    {
        // シーン内でポイントを消した時に反映したいなら残す（今のExcludeと同じ）
        var cell = WorldToCell(transform.position);
