...
public class PlayerSpawner : MonoBehaviour
{
    ...
    private IEnumerator SpawnPlayers()
    {
        do
        {
            ...
            // StartDirection を「スポナー(SpawnPoint)から見たスポーン位置」で決めるための基準
            Transform startDirOrigin = (spawnPoint != null) ? spawnPoint : this.transform;
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (cfs != null)
                {
                    // ★ CellFromStart 側が自動で StartDirection を決めるときの基準点を渡す
                    cfs.startDirectionOrigin = startDirOrigin;
                ...
                // ★ FrontierExplorer を使うPrefabのときも同様に基準点を渡す
                var fe = obj.GetComponent<FrontierExplorer>();
                if (fe != null)
                {
                    fe.startDirectionOrigin = startDirOrigin;
                }
            }
