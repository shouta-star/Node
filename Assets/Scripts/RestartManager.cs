...
public class RestartManager : MonoBehaviour
{
    ...
    [SerializeField] private float snapshotIntervalSec = 1f;
    ...
    void Start()
    {
        ...
        // ★ Run開始：NodeVisitログを初期化（Frame基準・RunIDを整える）
        EvaluationLogger.ResetNodeVisitLog();
        // ★ BaselineAgent の PlayerID を毎Run 1から振り直す
        BaselineAgent.ResetPlayerIdCounter();

        // ★ periodic snapshot initial
        snapshotSeq = 0;
    ...
    private void Update()
    {
        ...
        // 10秒に1回：色反映 → EndOfFrameでスクショ
        if (enablePeriodicSnapshots && !isRestarting && !snapshotInProgress)
        {
    ...
    //    // ファイル名が上書きされないように連番を付ける
    //    string tag = $"S{snapshotSeq:000}";
    //    snapshotSeq++;
    //    EvaluationLogger.CaptureRunScreenshot(tag);
    ...
    private IEnumerator CaptureSnapshotEndOfFrame()
    {
        ...
        try // ★ 外側は catch を付けず finally だけ（yield を入れてOK）
        {
            // 1) 色反映（ここは yield が無いので try/catch OK）
            try
            {
                //MapNode.ApplyColorsOnceBeforeScreenshot();
                MapNode.ApplyColorsBeforeScreenshot();          // passCount が増えた時だけ更新
            }
            catch (System.Exception ex)
            {
                Debug.LogError($"[RestartManager] ApplyColorsOnceBeforeScreenshot failed: {ex}");
            }

            // 2) 描画完了待ち（yield は try/catch 直下に置かない）
            yield return new WaitForEndOfFrame();

            // 3) スクショ（ここも yield が無いので try/catch OK）
            string tag = $"S{snapshotSeq:000}";
            snapshotSeq++;
            ...
            {
                Debug.LogError($"[RestartManager] CaptureRunScreenshot failed: {ex}");
            }
        }
        finally
        {
            // ★ これが目的：途中で落ちても必ず解除される
            snapshotInProgress = false;
        }
    }
    ...
    private void RecordEvaluation()
    {
        ...
        // ★ BaselineAgent がいる場合（CellFromStart がいないケース）
        var baseline = FindObjectOfType<BaselineAgent>();
        if (cfs == null && baseline != null)
        {
            uMode = "Baseline";
            tMode = baseline.mode.ToString();
            fMode = "None";
        }
    ...
    private IEnumerator RestartFlow()
    {
        ...
        var baselinePlayers = FindObjectsOfType<BaselineAgent>();
        foreach (var p in baselinePlayers) Destroy(p.gameObject);
        ...
        // ★ periodic snapshot reset (new run)
        snapshotSeq = 0;
        nextSnapshotTime = Time.time + Mathf.Max(0.1f, snapshotIntervalSec);
        snapshotInProgress = false;
        BaselineAgent.ResetPlayerIdCounter();
